...
In progress: V1 Phase 17 (17A–17C implemented on its branch)
...
Phase 16 delivers managed identity, BFF, ownership, organization, quota, account, consent, retention, and frontend foundations. Its detailed implementation record is archived in [`docs/archive/v1_phase_16/`](docs/archive/v1_phase_16/). Phase 17A–17C adds the durable job foundation, authenticated queue control plane, and internal fake-worker lifecycle proof on `agent/v1-phase-17-durable-jobs`; asynchronous analysis and provider execution remain intentionally unimplemented. The remaining live-provider and qualified legal release checks are tracked as final V1 Phase 22 work.
