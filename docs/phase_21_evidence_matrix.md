...
| 21D Research intelligence | Implemented locally; hosted exact-head CI passed on Draft PR #34 | Reversible `0032 -> 0033 -> 0032 -> 0033` SQLite/PostgreSQL evidence; immutable thesis revisions and append-only assumption versions with PostgreSQL current-head locking; bounded catalyst date precision/uncertainty; deterministic report comparison with checksummed inputs and same-scope authorization; citation-lineage current/superseded/deleted/unavailable/unknown signals without report rewriting; deterministic scenario deltas and bounded non-execution monitoring questions; thesis/account/organization lifecycle disposal and export; focused thesis/report UI/BFF contracts. No model route/task, provider activation, automatic schedule/notification, execution, wallet, custody, or advice capability. Hosted validation does not claim production activation. |
