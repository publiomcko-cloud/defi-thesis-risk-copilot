...
Status: **Closeout validation pending — 21A–21E are merged; 21F is recording full-regression and exact-head hosted evidence without adding capability.**
...
| 21E Worker compute/training governance | Merged through PR #35 at `bfc52be02cf1d11d8cac94f4cc4585611e8a4d52`; historical correction `2b3817e75f1a7130d847f0c4e58bc2aa94d3e0a9` | Reversible `0033 -> 0034 -> 0033 -> 0034` SQLite/PostgreSQL migration evidence for immutable `training_dataset_manifests`, `training_dataset_entries`, and `training_runs`; the sole `report_synthesis_training_synthetic_v1` source is checked-in synthetic material, sealed with content/manifest/held-out checksums, deterministic SHA-256 ranked train/validation/test splits, duplicate rejection, and an explicit approved eligibility result. Evaluation candidates receive only a neutral synthetic report plus actual untrusted/public scenario text: evaluator result/failure/category labels and case-specific fixture identifiers never enter the candidate request, and server-side quality detection remains evaluator-side. `report_synthesis.promotion.v3` binds that corrected evaluator protocol through `evaluation.visible-content.nfkc-whitespace-casefold.sha256.v1` and the code-owned input-isolation policy. Completed `report_synthesis.promotion.v2` runs remain immutable historical evidence but are obsolete: promotion, current routing, execution-route validation, and rollback restoration require the current v3 policy/checksum, exact prompt, and active ordinary plus adversarial corpus identity/version/checksum/count. A v2 route therefore falls back before provider use, including a captured snapshot; a rollback whose prior route is v2 clears to deterministic/no-model state. Instrumented ordinary and adversarial suite coverage checks every actual `LLMRequest`; semantic fake-provider modes activate only on the visible scenario text. The held-out checksum binds 21B/21C dataset IDs/versions and deduplicated candidate-visible component fingerprints under `evaluation.visible-content.nfkc-whitespace-casefold.sha256.v1`; Unicode NFKC, whitespace collapse, casefold, UTF-8, and SHA-256 reject held-out text copied under another training ID, including whitespace/case variants, without persisting raw evaluation bodies. Feedback, private user, and organization material are excluded before sealing. A dedicated `model.training.prepare.v1` Phase 17 job accepts only a server-created run identifier, carries an immutable server-owned execution snapshot, and uses the code-owned `local_fake_v1` profile with no provider, network, credentials, GPU, shell, or cost path. Durable global/user/provider reservation rows retain Phase 17 authority; the named profile scope permits one pending/running run and PostgreSQL contention accepts one submission while rejecting the other. Manifest/entry and run-snapshot database guards reject direct mutation. Worker lease/start/retry/cancel/dead-letter paths retain immutable evidence; terminal completion verifies the exact result and creates one checksummed model-card plus receipt metadata pair, while stale/forged completion is rejected. Runs are `not_registry_eligible`; no registry, candidate evaluation, promotion, route, production activation, real weights, real rental, or private-tenant training is created. Account disposal detaches the actor but preserves manifest, entries, and run evidence. The admin UI/API expose profile, manifests, and runs only; no source-body, image, offer, credential, or shell control is exposed. |
| 21F Closeout | Validation pending | Documentation-only reconciliation of complete SQLite/PostgreSQL migration lineage, backend/frontend/browser/Compose/security/Phase 19 evidence, and required exact-head hosted CI. See `phase_21_closeout.md`; Phase 22 remains deferred. |
...
Checkpoints 21A–21E have concrete implementation evidence. Phase 21F adds no
product capability; it is pending its own full-regression and exact-head hosted
evidence. No production activation is implied.
