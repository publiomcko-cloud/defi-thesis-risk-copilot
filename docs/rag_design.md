...
The current `knowledge_base/` Markdown tree and `.rag_index.json` remain global public-curated resources and the default rollback fallback. Analysis requests cannot provide an organization identifier. When the guarded Phase 18 primary flag is enabled, the backend derives active organization IDs and caller ownership from the authenticated actor and retrieves only approved public, caller-owned private, and active-organization durable chunks. Anonymous analysis remains public-only. Storage, ingestion, embeddings, shadow retrieval, and the primary flag remain disabled until Phase 22 production policy/cutover evidence is recorded.
