...
| Backend health/readiness/docs | `VERIFIED` | 2026-09-23T16:43Z public `GET /health`, `/ready`, and `/docs` each returned HTTPS 200; readiness reported database and RAG index true. | These safe endpoints do not establish candidate provenance, migration head, backup, or tenant boundaries. |
| Backend deployment status | `BLOCKED_EXTERNAL` | 2026-09-23T16:43Z public `GET /api/deployment/status` returned HTTPS 200 and safe production/public-demo/auth/database/disabled-provider fields, but commit was `6f07ef9`. | `6f07ef9` is an ancestor, not candidate `abc83d36`; no authorized Render CLI, API credential, deploy hook, or provider session is available. An authorized operator must deploy and prove the candidate. |
| Vercel candidate provenance | `VERIFIED` | GitHub deployment `6612919047` for exact SHA `abc83d36` completed `success` in Vercel `Production` at 2026-09-23T11:35:12Z. The production deployment alias is Vercel-auth protected, while canonical `/`, `/demo`, and `/status` each returned HTTPS 200 at 16:43Z. | The public canonical routes do not themselves disclose a commit; GitHub's Vercel deployment record is the provenance evidence. |
...
| Worker, real Vast, and model provider | `BLOCKED_EXTERNAL` | Public status reports Vast disabled/dry-run and model provider disabled. Scheduled Phase 17 worker run `35878131521` on candidate `abc83d36` failed during its claim/process step; no secret values were inspected. | No real rental/inference. An authorized operator must diagnose the deployed worker configuration and provide approved worker evidence separately. |
